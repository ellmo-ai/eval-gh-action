...
def remove_path_prefix(full_path, prefix):
    ...
    result = full_path[len(prefix)+1:]
    return f"./{result}"

def main():
    parser = argparse.ArgumentParser(description="Run evaluations based on changed files.")
    parser.add_argument('--env-vars', type=str, required=True, help='Environment variables in key:value format')
    ...
    parser.add_argument('--changed-files', type=str, required=True, help='List of changed files')
    ...
    prompts_path = config_content['prompts']['promptsPath']

    prompts_path = simplify_path(f"{working_directory}/{prompts_path}")
